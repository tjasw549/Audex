...
import SongsFilter from "../components/SongsFilter.jsx";
import SongsMain from "../components/SongsMain.jsx";

const SongsView = ({ addToPlaylist }) => {
  ...
  const { artist_id, genre_id } = useParams();
  ...
  useEffect(() => {
    ...
    fetch("https://comp4513-spotify-api.vercel.app/api/songs")
      ...
      .then((data) => {
        ...
        if (artist_id) setSelectedArtists([Number(artist_id)]);
        if (genre_id)  setSelectedGenres([Number(genre_id)]);

        setLoading(false);
      });
  }, [artist_id, genre_id]); // re-fetch if route params change

  const years = [...new Set(songs.map((s) => s.year))].sort();

  const artists = [
    ...new Map(songs.map((s) => [s.artists.artist_id, s.artists])).values(),
  ].sort((a, b) => a.artist_name.localeCompare(b.artist_name));

  const genres = [
    ...new Map(songs.map((s) => [s.genres.genre_id, s.genres])).values(),
  ...
  useEffect(() => {
    ...
    if (titleFilter)
      result = result.filter((s) => s.title.toLowerCase().includes(titleFilter.toLowerCase()));
    if (selectedYears.length)
      result = result.filter((s) => selectedYears.includes(s.year));
    if (selectedArtists.length)
      result = result.filter((s) => selectedArtists.includes(s.artists.artist_id));
    if (selectedGenres.length)
      result = result.filter((s) => selectedGenres.includes(s.genres.genre_id));
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-screen bg-[#060810]">
        <span className="font-['Bebas_Neue'] text-4xl tracking-[8px] text-[#00c8ff]/40 animate-pulse">
          LOADING...
        </span>
      </div>
    );
  ...
  return (
    <div className="bg-[#060810] overflow-x-hidden text-[#ddeeff] min-h-screen flex flex-col md:flex-row">
      <SongsFilter
